...
- 讨论 $f(x)$ 在指定区间 $I$ 上的有界性：
  ...
  - 若 $I$ 为 $(a,b)$ ，
    - $$\left. \begin{array} \text{\lim\limits_{x \to a^+}f(x) 存在} & \implies & \text{f(x) 在 a 的右邻域有界} \\ \lim\limits_{x \to b^-}f(x) 存在 & \implies & \text{f(x) 在 b 的左邻域有界} \\ & & \text{f(x) 在 (a,b) 内连续} \end{array} \right \} \implies f(x) 在 (a,b) 内有界 $$ 
    - 通常 $f(x)$ 在 $(a,b)$ 内连续必成立
      - 初等函数在其定义区间内必连续
...
- 解题思路
  ...
  - $\infty^0$ , $0^0$ , $1^\infty$ 
    ...
    - 例题
      ...
      - $\lim\limits_{ x \to \frac{\pi}{4} }(\tan x)^{ \dfrac{1}{\cos x - \sin x} }$ 
...
- 泰勒公式
  ...
  - 解题思路
    - $\dfrac{A}{B}$ 型
      ...
      - $\lim\limits_{x \to 0}\dfrac{\arcsin x - \arctan x}{x^3}$ 
    - $A - B$ 型
      - 见到 $A - B$ 型，用【幂次最低】原则
